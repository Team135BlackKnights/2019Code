...
public class DriveToward extends Command {
    ...
    protected void execute() {
        ...
        if (VALID_TARGET) {
            Robot.driveTrain.DriveStraightWVision(DRIVE_TRAIN_MOTOR_POWER, limelightData[Limelight.HORIZONTAL_OFFSET],
                    LIMELIGHT_DRIVE_STRAIGHT_P_VALUE);
        }
    }

    protected boolean isFinished() {
        return (limelightData[Limelight.TARGET_AREA] >= AREA_THRESHOLD) || VALID_TARGET == false;// ||
                                                                                                 // //Robot.ultra.leftSonar.getRangeInches()
                                                                                                 // <
                                                                                                 // DISTANCE_THRESHOLD;
    }
